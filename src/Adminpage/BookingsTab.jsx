...
const BookingsTab = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get('https://capstone-be-1.onrender.com/api/bookings/bookings',{
          headers: {
  ...
  const deleteBooking = async (bookingId) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.delete(`https://capstone-be-1.onrender.com/api/bookings/${bookingId}`,{
          headers: {
              Authorization: `Bearer ${user.token}`, // Include the token in the request headers
          },
      });
