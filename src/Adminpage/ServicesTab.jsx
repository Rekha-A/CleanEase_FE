...
const ServicesTab = () => {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get('https://capstone-be-1.onrender.com/api/cleaningservices/getAllCleaningServices/',{
          headers: {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`https://capstone-be-1.onrender.com/api/cleaningservices/${selectedService._id}`, updatedService,{
        headers: {
            Authorization: `Bearer ${user.token}`, // Include the token in the request headers
        },
    });
  ...
  const handleDelete = async (serviceId) => {
    try {
      await axios.delete(`https://capstone-be-1.onrender.com/api/cleaningservices/${serviceId}`,{
        headers: {
            Authorization: `Bearer ${user.token}`, // Include the token in the request headers
        },
    });
