...
const UsersTab = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get('https://capstone-be-1.onrender.com/api/users/users');
  ...
  const updateUserToAdmin = async (userId) => {
    try {
      await axios.put(`https://capstone-be-1.onrender.com/api/users/${userId}`, { isAdmin: true },{
        headers: {
  ...
  const deleteUser = async (userId) => {
    try {
      await axios.delete(`https://capstone-be-1.onrender.com/api/users/${userId}`,{
        headers: {
            Authorization: `Bearer ${user.token}`, // Include the token in the request headers
        },
    });
