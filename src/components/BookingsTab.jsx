    ...
    const fetchUserBookings = async () => {
      ...
      try {
        ...
        if (user && user.username) {
          const response = await axios.get(`https://capstone-be-1.onrender.com/api/bookings?username=${user.username}`,{
            headers: {
  ...
  const handleCancelBooking = async (bookingId) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.put(`https://capstone-be-1.onrender.com/api/bookings/${bookingId}`, { status: 'Cancelled' });
