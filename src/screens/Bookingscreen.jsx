...
const Bookingscreen = () => {
  ...
  useEffect(() => {
    const fetchServiceDetails = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await axios.get(`https://capstone-be-1.onrender.com/api/cleaningservices/getCleaningServiceById/${serviceid}`,{
          headers: {
  ...
  const handleSubmit = async (token) => {
    ...
    try {
      ...
      const existingBookingResponse = await axios.get(`https://capstone-be-1.onrender.com/api/bookings/bookings/checkAvailability`, {
        params: {
          serviceId: serviceid,
      ...
      const response = await axios.post('https://capstone-be-1.onrender.com/api/bookings/bookings', bookingData,{
        headers: {
            Authorization: `Bearer ${user.token}`, // Include the token in the request headers
        },
    });
